...
static UWORD32 F(BLOWFISH_CTX *Ctx,UWORD32 x) 
{
UWORD16	a; 
UWORD16 b; 
UWORD16 c; 
...
void Blowfish_Encrypt(BLOWFISH_CTX *Ctx,UWORD32 *xl,UWORD32 *xr) 
{
UWORD32	Xl;
UWORD32	Xr;
UWORD32	Temp;
...
void Blowfish_Decrypt(BLOWFISH_CTX *Ctx,UWORD32 *xl,UWORD32 *xr) 
{
UWORD32	Xl;
UWORD32	Xr;
UWORD32	Temp;
int     iCount;
	...
	for( iCount = N + 1; iCount > 1; --iCount ) 
	{
		Xl = Xl ^ Ctx->P[iCount];
		Xr = F(Ctx, Xl) ^ Xr;
...
void Blowfish_Init(BLOWFISH_CTX *Ctx,UBYTE *Key,INT32 KeyLen) 
{
UWORD32	uData; 
UWORD32	uDatal; 
UWORD32	uDatar;
...
int		k;
	...
	for( i = 0; i < N + 2; i += 2 ) 
	{
		Blowfish_Encrypt(Ctx,&uDatal,&uDatar);
		Ctx->P[i] = uDatal;
		Ctx->P[i + 1] = uDatar;
	}
...
char	szBinKey[16];
	...
	GenerateMD5Key(ArcStruct.szHeadPswd,szBinKey);
	memset(&ArcStruct.BfCtx,0,sizeof(BLOWFISH_CTX));
	Blowfish_Init(&ArcStruct.BfCtx,(UBYTE *)szBinKey,(INT32)sizeof(szBinKey));
...
UWORD32 uCount;
	...
	for( uCount = 0; uCount < uBufSize; uCount += BLOWFISH_BYTE_BLOCKLEN )
		Blowfish_Decrypt(&ArcStruct.BfCtx,(UWORD32 *)&pBuffer[uCount],(UWORD32 *)&pBuffer[uCount + sizeof(UWORD32)]); 
}
