...
unsigned int i, index, partLen;
	...
	if (inputLen >= partLen) 
	{
		memmove(&context->buffer[index],input,partLen);
  ...
  //- Buffer remaining input 
	memmove(&context->buffer[index],&input[i], inputLen-i);
}
...
char	szTempKey[276];
	...
	strncpy(szTempKey,szUserKey,sizeof(szTempKey) - 1);
	szTempKey[275] = '\0';
	MD5Init(&MD5Context);
	MD5Update(&MD5Context,szTempKey, strlen(szTempKey));
	//- TODO
	MD5Final(szMD5Key16, &MD5Context);
}
